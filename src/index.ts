/**
 * dsh-mobile, node half: mounts the upload RPC channel (PRD 3.4) on the
 ...
 * `/dsh-mobile/<endpoint>` with the connection transport's own trust fence
 * (DNS-rebinding + cross-site defense over the deployment's trustedHosts).
 * @param ctx - host context.
 */
export function apply(ctx: Context): void {
  const channel = createUploadChannel()
  const remove = ctx.connection.rpc.handle('/dsh-mobile', channel.handler, { authority: 'trusted-host' })
  ctx.effect(() => () => {
    channel.dispose()
    void remove()
  }, 'dsh-mobile: upload channel')
}
